...
Version: v27.10-email-design-followup-automation-max3-stable
...
## Latest Critical Update — v27.10 Email Design Stability, Automation Follow-up Polish, and Max-3 Follow-up Safety

This section is now the highest-priority operating rule for future work on TrackFlow Pro email design, Outlook/Gmail/mobile rendering, Automation follow-up templates, and follow-up stop rules.

### Important ownership/context note

This README is a project context handoff file only.

It does **not** contain the full project source code, Python backend files, all folders, dependencies, or production secrets.

Use it to help a future ChatGPT/developer understand the project quickly. For any real patch, the owner must still upload the current related source files from the actual project.

Correct future workflow:

```text
1. Upload this README.
2. Explain the specific bug/change.
3. Upload only the current related files from the project.
4. Assistant inspects those files before patching.
5. Assistant returns a ZIP with exact project-relative paths and README_FIX.txt.
```

Do not assume a past patch is already applied. Always inspect the current uploaded files.

### Recent email design stabilization

Recent email design work fixed Outlook/Gmail/mobile rendering issues.

Confirmed problem patterns:

```text
Outlook desktop was clipping or breaking the report CTA button.
Email body line spacing became too large when the composer used Enter/new paragraphs.
Follow-up signature needed to look clean on desktop, mobile, Gmail, and Outlook.
```

Stable design rules:

```text
1. Prefer Outlook-safe table-based email layout.
2. Avoid fragile VML rounded buttons when Outlook clips button height/border.
3. Do not use aggressive promotional button styling for cold outreach.
4. Use a subtle table/button approach that renders acceptably across Gmail, Outlook desktop, Outlook web, mobile clients, and in-app browsers.
5. Normalize email body HTML before sending so empty paragraphs do not create huge gaps.
6. Keep paragraph spacing compact and natural.
7. Keep signature and unsubscribe/reference line visible.
8. Keep the open tracking pixel hidden only.
```

Email body cleanup should remove or compact patterns like:
...
But it must not destroy normal paragraphs, links, bold/italic, merge tags, or safe HTML.

### Initial outreach email signature rule

Initial outreach emails may use the fuller signature.

Preferred full signature service line:

```text
Google Ads Conversion Tracking · GA4/GTM Audit · Server-Side Tracking
```

Older line that may still appear in older deployments/emails:

```text
Google Ads Tracking · Server-Side Tracking · Conversion Audit
```

If this older line appears only in an old already-sent email, it is not a bug. Email HTML is static after sending.

Connected files for initial email design/signature:
...
Only touch `OutreachPanel.tsx` if dashboard preview/composer rendering must match the sent email. If the actual sent email is the only issue, the catch-all route may be enough.

### Automation follow-up is separate from Outreach composer

Important distinction:

```text
Outreach panel = initial email composer/send workflow.
Automation panel = follow-up templates/settings/scheduling workflow.
```

Do not confuse them.

When the owner says "follow-up", first inspect the Automation-related files unless the screenshot/problem clearly belongs to the initial outreach composer.

Connected follow-up UI/config files:
...
Connected follow-up backend/send/schedule file:

```text
app/api/trackflow/[...action]/route.ts
```

### Follow-up copy strategy

Follow-ups go in the original reply thread when threading is working. Because the initial email already contains the secure report CTA/button, follow-ups should **not** auto-add the secure page URL/button.

Correct behavior:

```text
1. Follow-up emails should be short, natural, and reply-thread friendly.
2. Do not auto-add "The private note is also here..." or "View short audit note" to follow-ups.
3. If the owner wants a follow-up to include the secure page link, the owner can manually write that inside the follow-up template.
4. Default follow-ups should refer to the note sent above / tracking note.
5. Use {company} when useful.
6. Avoid sounding like a new separate marketing email.
```
...
Just checking if you had a chance to look at the tracking note I sent for {company}.

It was not a final diagnosis — just a browser-visible signal worth checking inside Google Ads, GA4, GTM, or the server-side setup.

If helpful, I can point out the first area I would verify so you do not have to dig through everything.
```

Do not automatically add:

```text
The private note is also here if you want to recheck it: View short audit note
```

### Automation follow-up signature rule

Follow-up signature should be compact, not a full promotional block.

Preferred compact signature idea:

```text
Sender Name
TrackFlowPro · Conversion Tracking Audit
shahjalal@trackflowpro.com | trackflowpro.com
Reference: TF-... | Unsubscribe
```

Rendering rules:

```text
1. Use Outlook-safe table layout.
2. Keep compact font sizes and line-height.
3. Allow wrap on mobile.
4. Keep reference/unsubscribe visible.
5. Do not hide compliance/reference text.
6. Do not add buttons to the follow-up signature.
```

### Max follow-up policy — hard stop at 3
...
Only 3 follow-ups should ever be sent to one lead.

Do **not** rely on blank F-4/F-5 text boxes to stop follow-ups because older code/config can fall back to code-default step4/step5 templates.

Correct implementation:

```text
1. Backend must have a code-level max follow-up limit of 3.
2. Frontend status calculation must also treat follow_up_count >= 3 as done.
3. Automation UI should show/configure only F-1, F-2, F-3.
4. If an old lead has nextFollowupStep 4 or 5 scheduled, the backend should not send it.
5. After F-3 is sent/evaluated, lead should move to a finished/stopped-safe state.
6. Open/click after F-3 must not schedule F-4/F-5.
```

Recommended constants/policy naming:

```text
MAX_FOLLOWUP_STEPS = 3
FOLLOWUP_ACTIVE_STEPS = ["step1", "step2", "step3"]
```

`StepId` may still include `step4` and `step5` for old Firestore data compatibility, but active automation should only expose/use the first 3 steps.

### Safety requirements for Max-3 follow-up changes

This area is critical. It previously had bugs. Future assistants must be very careful.

When changing the max follow-up policy, preserve:
...
Sender validation
Open/click tracking
Open/click dedupe and provider-image proxy rules
Follow-up reply threading headers
Firestore lead identity / tracking ids
Secure report/PDF/B2/Supabase systems
Google Sheet queue logic
Suppression/unsubscribe logic
```

Do not refactor the whole catch-all route just to change the max follow-up count.

Safe change strategy:

```text
1. Add/adjust a small max-followup constant near existing follow-up constants.
2. Replace only the existing follow-up-count >= 5 behavior with >= 3 where send/schedule/status logic needs it.
3. Add guard before sending follow-ups so step4/step5 cannot send even if already scheduled.
4. Keep old step4/step5 types/config tolerated but inactive.
5. Keep UI consistent by exposing only active steps.
6. Do not delete historical sent_messages or old Firestore config.
```

### Recent patch handoff names for this area

Recent related patch ZIPs:

```text
trackflow-email-platform-stable-design-fix.zip
trackflow-followup-automation-polish-fix.zip
trackflow-followup-max3-safe-fix.zip
```

Purpose:

```text
email-platform-stable-design-fix:
Outlook-safe CTA/button rendering, compact body spacing cleanup, platform-safe email design.

followup-automation-polish-fix:
Automation follow-up copy polish, compact follow-up signature, no auto secure-page link/button in follow-ups.

followup-max3-safe-fix:
Hard stop follow-up automation after F-3, UI exposes only F-1/F-2/F-3, old F-4/F-5 should not send.
```

These are history notes only. Future assistants must inspect the current uploaded files before assuming the patch is applied.

### Test checklist after applying email/follow-up patches

Email design test:

```text
1. Send a fresh initial email after deployment.
2. Check Gmail web.
3. Check Outlook desktop.
4. Check Outlook web if available.
5. Check mobile Gmail/Outlook.
6. Confirm CTA button is not clipped.
7. Confirm body spacing is compact after using Enter/new paragraphs in the composer.
8. Confirm signature wraps cleanly on mobile.
9. Confirm reference/unsubscribe line is visible.
10. Confirm open/click tracking still works.
```

Follow-up automation test:

```text
1. In Automation tab, confirm only F-1, F-2, F-3 are visible/configurable.
2. Save follow-up settings.
3. Confirm no F-4/F-5 UI/editor is exposed.
4. Trigger/test F-1 and confirm it goes inside the original email thread.
5. Confirm follow-up email has no auto secure-page button/link unless manually written in the template.
6. Trigger/test F-2 and F-3 if using a test lead.
7. After F-3, confirm the lead is finished/stopped-safe.
8. Confirm a later open/click does not schedule F-4/F-5.
9. If an old lead had F-4/F-5 scheduled, confirm cron does not send it.
10. Check Firestore lead fields: follow_up_count, nextFollowupStatus, nextFollowupStep, sent_messages.
```

Build honesty:

```text
Patches created from uploaded partial project files may only have changed-file TypeScript syntax/transpile checks.
Full npm run build is only valid if run in the complete project with dependencies.
After applying any patch, run:
npm run lint
npm run build
```
...
## Latest Critical Update — v26.50 Secure Report Lightweight Intent Tracking, Device Summary, Dashboard Source Filters, and Stale Event Guard

This section is now the highest-priority operating rule for future work on secure report visitor activity tracking, device summary, stale browser-tab event safety, source filters, and the Saved Secure Reports list in the dashboard.
...
What was the latest visitor device, browser, and OS summary?
Was the first visitor device different from the latest device?
...
raw request headers
raw full user-agent strings
raw IP address or precise visitor identifier
full device history arrays
third-party iframe logs
```

### Device / visitor summary rule

Device data is useful for follow-up prioritization, but it must stay lightweight and privacy-safe.

Correct behavior:

```text
First/original device summary:
visitorDeviceType
visitorBrowser
visitorOs

Latest visit device summary:
lastVisitorDeviceType
lastVisitorBrowser
lastVisitorOs
```

If the same report is first opened on desktop and later opened on mobile:

```text
visitorDeviceType = desktop
lastVisitorDeviceType = mobile
```

The dashboard should display the latest device first, and show the first device only when it is different:
...
Allowed values should be normalized labels only, for example:
...
Never store the raw user-agent, raw IP address, raw headers, or a device history array. If deeper debugging is needed, use temporary server logs only and remove/disable them after the issue is fixed.

Connected files for device summary:
...
Avoid touching `app/components/trackflow/SecureReportAnalytics.tsx` for device display-only bugs, because this component contains GA4/report-event client tracking logic.
...
### Deleted / cleaned report stale-event guard

A removed/cleaned report must not be recreated or updated by an old browser tab.

Known bug pattern:

```text
1. Operator removes/cleans a report.
2. The secure report page was still open in a browser tab.
3. The tab is refreshed or sends a final heartbeat.
4. /api/report-event receives secure_report_duration_final.
5. Firestore audit_reports/{token} gets written again with fields like:
   estimatedActiveSeconds
   lastActivityAt
   lastActivityEvent
   lastIntentLabel
...
Correct server-side guard:

```text
Before writing any lightweight summary fields to audit_reports/{token}:

1. Read the existing audit_reports/{token} document.
2. If the document does not exist, do not create it from a tracking event.
3. If active === false, skip the Firestore summary write.
4. If cleanupStatus/status indicates cleaned or deleted, skip the Firestore summary write.
5. Valid active reports should continue to update normally.
```

Guarded cleanup statuses should include:
...
```text
This guard should only stop Firestore summary writes for missing/cleaned/deleted reports.
Do not break GA4 event forwarding for valid active reports.
Do not touch SecureReportAnalytics.tsx unless the client event payload itself is wrong.
Do not change cleanup execution logic just to fix stale browser-tab activity.
```

If a similar stale activity bug appears from `/api/trackflow/reports/view`, inspect the catch-all route carefully, but only touch the report-view visitor summary section.
...
Row index such as #1, #2, #3 for quick reference
```

The goal is not to create a complex analytics dashboard. The goal is to make follow-up decisions faster.

Current preferred desktop layout:

```text
[ Lead identity + source ]   [ Activity + contact ]   [ Visitor / device / intent / actions ]
```

The row should avoid stacking all information on the left while leaving the right side empty. On mobile, the same information can collapse into a single-column layout.

Visitor card display rule:

```text
Country
Device latest + first device when different
Active time
Last seen
Expiry
Intent
...
Do not truncate important `Last seen` or `Expiry` values inside tiny boxes. If needed, show those values as full-width rows.
...
Important final filter rule:

```text
The badge label and the filter result must agree.
If a row displays Search / Email, it must appear under Search / Email leads.
If a row displays Manual report or LinkedIn report, it must appear under LinkedIn / Manual reports.
```

Both backend and frontend filters should honor explicit fields first:
...
Do not rely only on one field when the row badge is built from a wider source classification.
...
Firestore read-safety rule:

```text
Default list limit should be 25, not 100.
The UI should also display only the limited visible rows unless a deliberate Load more/pagination flow is added.
Do not load hundreds of audit_reports documents on every Cleanup tab refresh.
```

If future pagination is added, prefer an explicit Load more button and cursor-based reads instead of fetching all reports at once.
...
12. Confirm row shows country, device, active time, PDF, video, chat, CTA, and intent.
13. Open the same report from desktop first, then mobile. Confirm dashboard shows latest device and First: Desktop where applicable.
14. Clean/delete a test report while its secure page is still open in another tab.
15. Refresh the old tab and confirm `/api/report-event` does not recreate/update `audit_reports/{token}`.
```
...
trackflow-cleanup-premium-report-rows.zip
trackflow-cleanup-source-filter-read-limit-fix.zip
trackflow-cleanup-visitor-device-summary-fix.zip
trackflow-cleanup-device-first-latest-display-fix.zip
trackflow-cleanup-source-filter-final-fix.zip
trackflow-report-event-stale-cleanup-guard-fix.zip
```
...
premium-report-rows: Make Cleanup Saved Secure Reports rows use a compact premium layout, row index numbers, full right-side visitor summary, and non-truncated date/expiry display.
source-filter-read-limit-fix: Reduce default Cleanup report list reads from 100 to 25 and improve Search/Email filter fallback.
visitor-device-summary-fix: Capture safe lightweight device/browser/OS summary without storing raw user-agent or IP.
device-first-latest-display-fix: Preserve first/original device while updating latest device, and display latest plus First: when different.
source-filter-final-fix: Make backend/frontend source filters agree with row badges and honor sourceGroup/sourceLabel/source fields consistently.
report-event-stale-cleanup-guard-fix: Prevent stale browser tabs from recreating or updating cleaned/deleted audit_reports via /api/report-event duration/final events.
```
