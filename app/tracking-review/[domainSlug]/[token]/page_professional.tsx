...
type SpeedSnapshot = {
  ...
  label?: string;
  visualLoadEstimateSeconds?: number | string | null;
  domContentLoadedSeconds?: number | string | null;
  networkIdleSeconds?: number | string | null;
  requestCount?: number | string | null;
  thirdPartyRequestCount?: number | string | null;
  uniqueHostCount?: number | string | null;
  note?: string;
};
...
function normalizeProblemCards(value: unknown, fallbackEvidence: string[] = []): ProblemCard[] {
  const rawItems = Array.isArray(value) ? value : [];
  const output: ProblemCard[] = [];
  const seen = new Set<string>();

  for (const item of rawItems) {
    const record = item && typeof item === "object" && !Array.isArray(item) ? (item as Record<string, any>) : {};
    const title = cleanText(record.title || record.problem || record.name || record.label, "");
    const finding = cleanText(record.finding || record.summary || record.description || record.text, "");
    const businessMeaning = cleanText(
      record.businessMeaning || record.business_impact || record.businessImpact || record.whyItMatters || record.impact,
      "",
    );
    const nextCheck = cleanText(record.nextCheck || record.next_check || record.manualCheck || record.recommendation, "");
    const evidence = cleanList(record.evidence || record.evidencePoints, [], 3);
  ...
  return fallbackEvidence.slice(0, 3).map((item, index) => ({
...
function getSpeedSnapshot(report: Record<string, any>, privateCopy: Record<string, any>): SpeedSnapshot | null {
  const raw =
    privateCopy.websiteSpeed ||
    privateCopy.website_speed ||
    report.websiteSpeed ||
    report.website_speed ||
    report.speed ||
    null;

  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
  return raw as SpeedSnapshot;
}

function speedMetricText(label: string, value: unknown, suffix = ""): string {
  const text = cleanText(value, "");
  if (!text) return "";
...
function ProblemCardsSection({ cards }: { cards: ProblemCard[] }) {
  if (!cards.length) return null;

  return (
    <SectionCard label="Business problems to verify" tone="amber">
      <div className="grid gap-4">
        {cards.map((card, index) => (
          <article
            key={`${card.title}-${index}`}
            className="rounded-[1.35rem] border border-amber-100 bg-white p-5 shadow-sm"
          >
            <div className="flex items-start gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-amber-100 text-sm font-black text-amber-800">
                {index + 1}
              </div>
              <div className="min-w-0">
                <h3 className="text-base font-black leading-6 text-slate-950">{card.title}</h3>
                <p className="mt-2 text-sm font-bold leading-7 text-slate-700">{card.finding}</p>
                <p className="mt-3 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-semibold leading-7 text-amber-950">
                  {card.businessMeaning}
                </p>
                <p className="mt-3 text-xs font-black uppercase tracking-[0.16em] text-amber-700">
                  Recommended verification
                </p>
                <p className="mt-1 text-sm font-semibold leading-7 text-slate-600">{card.nextCheck}</p>
                {card.evidence?.length ? (
                  <ul className="mt-3 space-y-2">
                    {card.evidence.map((item) => (
                      <li key={item} className="text-xs font-semibold leading-5 text-slate-500">
                        Evidence: {item}
                      </li>
                    ))}
                  </ul>
...
function SpeedSnapshotCard({ speed }: { speed: SpeedSnapshot | null }) {
  if (!speed) return null;

  const items = [
    speedMetricText("Score", speed.score),
    speedMetricText("Label", speed.label),
    speedMetricText("Visual load", speed.visualLoadEstimateSeconds, "s"),
    speedMetricText("DOM loaded", speed.domContentLoadedSeconds, "s"),
    speedMetricText("Network idle", speed.networkIdleSeconds, "s"),
    speedMetricText("Requests", speed.requestCount),
    speedMetricText("Third-party requests", speed.thirdPartyRequestCount),
    speedMetricText("Unique hosts", speed.uniqueHostCount),
  ].filter(Boolean);

  if (!items.length && !speed.note) return null;

  return (
    <SectionCard label="Website speed snapshot">
      <p className="text-sm font-semibold leading-7 text-slate-600">
        This is a lightweight browser-visible speed snapshot from the audit run. A full Core Web Vitals review should be confirmed separately when speed is a concern.
      </p>
      {items.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{items.map((item) => (
        <div key={item} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm font-black text-slate-800">
          {item}
        </div>
      ))}</div> : null}
      {speed.note ? <p className="mt-4 text-xs font-semibold leading-6 text-slate-500">{speed.note}</p> : null}
    </SectionCard>
  );
}
...
export default async function ReportPage({ params }: ReportPageProps) {
  ...
  const problemCards = normalizeProblemCards(
    privateReportCopy.problemCards ||
      privateReportCopy.businessProblems ||
      report.problemCards ||
      report.businessProblems,
    proofPoints,
  );
  const speedSnapshot = getSpeedSnapshot(report, privateReportCopy);
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      >
        <div className="space-y-6">
          ...
          <ProblemCardsSection cards={problemCards} />

          <SpeedSnapshotCard speed={speedSnapshot} />
