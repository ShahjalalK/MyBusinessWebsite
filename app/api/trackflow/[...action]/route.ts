...
const TRACKFLOW_CALENDLY_URL = (
  process.env.NEXT_PUBLIC_TRACKFLOW_CALENDLY_URL ||
  process.env.TRACKFLOW_CALENDLY_URL ||
...
function isReportTrackingReviewQuestion(question: string): boolean {
  const text = normalizeIntentText(question);

  if (!text) return false;

  const explicitContactOrSalesIntent =
    /\b(calendly|free consultation|verification call|book\s+(?:a\s+)?call|schedule\s+(?:a\s+)?call|book\s+(?:a\s+)?meeting|schedule\s+(?:a\s+)?meeting|book\s+(?:a\s+)?consultation|schedule\s+(?:a\s+)?consultation|contact\s+(?:trackflow|you|your team|support)|how\s+(?:can|do)\s+(?:i|we)\s+contact|contact email|reach\s+(?:you|trackflow)|email\s+(?:you|trackflow)|talk\s+to\s+(?:you|someone|a human)|speak\s+(?:to|with)\s+(?:you|someone|a human)|hire|work\s+with\s+you|work\s+together|get\s+started|start\s+working|continue\s+with\s+you|can\s+you\s+fix\s+this\s+for\s+us|can\s+you\s+verify\s+this\s+for\s+us|can\s+trackflow\s+pro\s+verify\s+this\s+for\s+us|want\s+to\s+proceed|next\s+step\s+to\s+work|fiverr|upwork|marketplace|freelance platform|platform[-\s]?based)\b/.test(text);

  if (explicitContactOrSalesIntent) return false;

  return /\b(reviewed|review|browser[-\s]?side|browser[-\s]?visible|visible|observed|evidence|tracking|conversion|journey|path|signal|signals|tag|tags|event|events|ga4|gtm|google ads|debugview|diagnostics|actual tracking accounts?|account[-\s]?level|confirm(?:ed)?|verification|verify|verified|what should be confirmed|what should be checked|what should we check|what should we verify|phone call|call[-\s]?click|phone[-\s]?click|click[-\s]?to[-\s]?call|contact us page|contact page|contact form|form submission|lead path|lead event|lead tracking|server logs?|call[-\s]?tracking|crm)\b/.test(text);
}
...
function isContactBookingQuestion(question: string): boolean {
  ...
  // Report-evidence questions can include words like "call", "contact", "booking",
  // or "appointment" as conversion paths. Answer those from the report first;
  // only use the contact/Calendly path when the visitor explicitly asks to reach
  // or hire TrackFlow Pro.
  if (isReportTrackingReviewQuestion(question)) return false;

  const asksContact =
    /\b(contact\s+(?:trackflow|you|your team|support|someone|a human)|how\s+(?:can|do)\s+(?:i|we)\s+contact|contact email|reach\s+(?:you|trackflow)|email\s+(?:you|trackflow)|mail\s+(?:you|trackflow)|message\s+(?:you|trackflow)|talk\s+to\s+(?:you|someone|a human)|speak\s+(?:to|with)\s+(?:you|someone|a human)|human support)\b/.test(text);

  const asksBooking =
    /\b(book|booking|schedule|calendly|calendar|meeting|consultation|consult|appointment|free consultation|verification call)\b/.test(text);
...
function buildContactBookingAnswer(context: AnyRecord, question: string): string {
  ...
  const asksBooking = /\b(book|booking|schedule|calendly|calendar|meeting|consultation|consult|appointment|free consultation|verification call)\b/.test(text);
