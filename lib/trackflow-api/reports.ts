...
export function createReportHandlers(deps: ReportHandlerDeps) {
  ...
  async function handleReportRegister(req: Request) {
    ...
    const payload: AnyRecord = {
      ...
      ogImageUrl: report.ogImageUrl || deleteField,
      openGraphImageUrl: report.openGraphImageUrl || report.ogImageUrl || deleteField,
      previewImageUrl: report.previewImageUrl || report.ogImageUrl || deleteField,
      homepageScreenshotUrl: report.homepageScreenshotUrl || report.ogImageUrl || deleteField,
      ogImagePathname: report.ogImagePathname || deleteField,
      ...
      // v18.23: remove old duplicated/heavy fields from previous report documents.
